...
class Main(BaseClass):
    ...
    def add_account():
        ...
        id_name = self.string_common.random_string(32,upper=False)
    ...
    def login_manager():
        ...
        for account in accounts:
            ...
            th = self.thread_common.get_thread(id_name)
            if th == None:
                is_login = False
            else:
    ...
    def get_login_verify():
        ...
        if th == None:
            ...
            time.sleep(5)
            vcode_close = self.selenium_common.is_element(".vcode-close", is_beautifulsoup=False)
            print(vcode_close)
            return None
        ...
        vcode_spin_img = self.selenium_common.find_element(".vcode-spin-img", is_beautifulsoup=False)
        if self.selenium_common.is_element(vcode_spin_img):
            ...
            return result
        try:
            token_img = self.selenium_common.find_element("#token-img", is_beautifulsoup=False)
        except:
            token_img = None
        if self.selenium_common.is_element(token_img):
    ...
    def api():
        ...
        if operator.__eq__(datatypes,"add_account"):
            ...
            data_temp = self.db_common.unserialization(data_time_tempfile)
            ...
            if th.login_check(init_check=True):
                ...
                th.quit()
                self.db_common.serialization(data,data_time_tempfile)
                self.thread_common.remove_thread(thread_name=id_name)
                print(f"data{data}")
                return data
            else:
                data = {
    ...
    def delete_account_to_account(self, user):
        ...
        self.db_common.serialization(accounts, filename,override=True)
    ...
    def account_browser_thread_create(self,allUsers=True):
        ...
        for id in range(account_max_thread):
            ...
            #     # config[login_k] = login_v
            # args = (config)
            th = self.thread_common.create_thread(thread_type="selenium",thread_name=id_name, args=config )
