...
class WebdownThread(threading.Thread, BaseClass):
    # 资源锁
    ...
    def run(self):
        ...
        return
        self.com_selenium.bind(self)
        __empty_url = "data:,"
        # 将初始url添加到队列。
        # TODO 开发url_wrap函数正常
        self.set_base_url()
        # 首次提交的网页也先加入队列，该方便对比重复
        url_wrap = self.url_wrap(url=self.__base_url, tag="<a>", ele=None)
        print(url_wrap)
        url = "/resource/view/5911"
        url_wrap = self.url_wrap(url=url, tag="<a>", ele=None)
        print(url_wrap)
        url = "/../view/5911"
        url_wrap = self.url_wrap(url=url, tag="<a>", ele=None)
        print(url_wrap)
    ...
    def find_all_tab_and_replace_html(self, tagnames):
            ...
            for atr in attrs:
                ...
                for ele in eles:
                    ...
                    if is_a_link:
                        ...
                        url_format = self.url_wrap(url, "<a>", None)
        ...
        url_web = self.url_wrap(url_web, "<a>", None)
